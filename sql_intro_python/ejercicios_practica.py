...
def insert_estudiante(new_student):
    conn = sqlite3.connect('secuendaria.db')
    c = conn.cursor()
    values = new_student
    c.execute("""
    INSERT INT0 estudiante (name, age, grade, tutor)
    VALUES (?,?,?,?);""", values)
    conn.commit()
    conn.close()
...
if __name__ == '__main__':
    ...
    group = [('Victoria', 13, 6, 'MarcelaRestrepo'),
                ('Dario', 57, 7, 'RosalbaSoto'),
                ('Uxia', 65, 7,'RosalbaSoto'),
                ('VeraCenalmor',67, 9,'RosalbaSoto'),
                ('LauraRestrepo', 87,9,'RosalbaSoto'),
                ]
    ...
    new_student = ['Ramiro', 41, 6, 'vacante']
